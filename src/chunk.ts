...
import {unpackArray_64BEA, PackedArray_64BEA_Builder} from "./packedArray64BEA";
import {packedArrayI8_4_get, packedArrayI8_4_set} from "./packedArray8";
import {intdiv, newarr} from "./util";
...
class Section{
    ...
    modified = false;
    getBlock(x,y,z){
        const idx = (y&15)*256+(z&15)*16+(x&15);
        return this.blockArr[idx];
    }
    setBlock(x,y,z,data){
        const idx = (y&15)*256+(z&15)*16+(x&15);
        this.modified = true;
        this.blockArr[idx] = data;
    }
    //todo: test these methods
    //non iterative base methods
    getBlockLight(x,y,z){
        if(!this.nbt.BlockLight)this.nbt.BlockLight = new Int8Array(2048);
        const idx = (y&15)*256+(z&15)*16+(x&15);
        return packedArrayI8_4_get(this.nbt.BlockLight,idx);
    }
    setBlockLight(x,y,z,value){
        if(!this.nbt.BlockLight)this.nbt.BlockLight = new Int8Array(2048);
        const idx = (y&15)*256+(z&15)*16+(x&15);
        packedArrayI8_4_set(this.nbt.BlockLight,idx,value);
    }
    getSkyLight(x,y,z){
        if(!this.nbt.SkyLight)this.nbt.SkyLight = new Int8Array(2048);
        const idx = (y&15)*256+(z&15)*16+(x&15);
        return packedArrayI8_4_get(this.nbt.SkyLight,idx);
    }
    setSkyLight(x,y,z,value){
        if(!this.nbt.SkyLight)this.nbt.SkyLight = new Int8Array(2048);
        const idx = (y&15)*256+(z&15)*16+(x&15);
        packedArrayI8_4_set(this.nbt.SkyLight,idx,value);
    }
...
class BlockEntities{
    ...
    constructor(nbt){
        ...
        for(let entity of nbt){
            ...
            entities.set(x&15,y,z&15,entity);
        }
    }
    set(x,y,z,props){
        const entity = {};
        ...
        this.entities.set(x&15,y,z&15,entity);
    }
    get(x,y,z){
        return this.entities.get(x&15,y,z&15);
    }
    delete(x,y,z){
        this.entities.delete(x&15,y,z&15);
    }
    toNBT(){
        const res = [];
...
export class Chunk{
    ...
    init(nbt){
        ...
        this.ymin = (nbt.sections[0]?.Y?.value ?? nbt.yPos.value)*16;
        this.ymax = this.ymin+this.sections.length*16;
    ...
    // Block level modification methods
    modified = false;
    getBlock(x,y,z){
